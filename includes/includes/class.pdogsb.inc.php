...
class PdoGsb
{
  ...
  public function majFraisForfaitHdd($idVisiteur, $mois , $idFrais,$libelle,$montant,$date)
    ...
    public function estPremierFraisMois($idVisiteur, $mois)
    {
        ...
        $requetePrepare->bindParam(':unMois', $mois, PDO::PARAM_STR); //Représente les types de données CHAR, VARCHAR ou les autres types de données sous forme de chaîne de caractères SQL. 
        $requetePrepare->bindParam(':unIdVisiteur', $idVisiteur, PDO::PARAM_STR);
        $requetePrepare->execute();
  ...
  return $lesMois;
    ...
    {     
        ...
        $requetePrepare = PdoGSB::$monPdo->prepare(
           'SELECT fichefrais.mois AS mois FROM fichefrais '
           . 'WHERE fichefrais.idvisiteur = :unIdVisiteur '
          . 'AND fichefrais.idetat = :unetatFiche '
            . 'ORDER BY fichefrais.mois desc'
       );
            }else{
             $R= "MP";
           ...
           'SELECT fichefrais.mois AS mois FROM fichefrais '
           . 'WHERE fichefrais.idvisiteur = :unIdVisiteur '
          . 'AND fichefrais.idetat = :unetatFiche OR fichefrais.idetat = :remb '
            . 'ORDER BY fichefrais.mois desc'
        ); 
            $requetePrepare->bindParam(':remb',$R , PDO::PARAM_STR);     
            }
                
        $requetePrepare->bindParam(':unIdVisiteur', $idVisiteur, PDO::PARAM_STR);
       $requetePrepare->bindParam(':unetatFiche',$etat , PDO::PARAM_STR);
        $requetePrepare->execute();
        $lesMois = array();
           $i=0;
